// Go (Golang) is a compiled programming language developed by Google for fast, simple, and scalable systems.
// It’s known for built-in concurrency (goroutines/handling multiple tasks/request at same time) and easy deployment with single binaries.
...
// In Go, every file belongs to a package. A package is like a folder of related code 
// package main is used to define the package package main specifically tells Go that this file
// is part of the main executable program (not a reusable library).
// main is a special package that tells Go this file is an executable program, not a library.
// When you run go build or go run, Go looks for the main package and the main() function to start execution.
...
func main(){ // func main is the entry point of the program
	...
	fmt.Print("Hello World") // Calls the Print function from the fmt package to display "Hello World" on the screen.
